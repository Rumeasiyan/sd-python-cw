...
def get_all_credits():      
    ...
    return no_of_pass, no_of_defer, no_of_fail
...
def Multiple_incomes():
    ...
    while True:
        ...
        if (loop_for_staff.upper() == 'Y') :    # Want to loop
            no_of_pass, no_of_defer, no_of_fail = get_all_credits()
            
            student_progress = progression_outcome(no_of_pass, no_of_defer, no_of_fail)
            if total_incorrect[0] != 1:
                students_progress.append([student_progress, no_of_pass, no_of_defer, no_of_fail])
